---
title: "Bayesian Variable Selection: Model I"
...
nIter <- ceiling((numSavedSteps*thinSteps )/nChains) 	#steps per chain
  ...
  params <- c("beta")
   ...
   inits1 <- list("alpha0" = sort(c(rnorm(1,log(1/2),.5), rnorm(1,log(2/1),.5))), "beta" = rep(0,P))
  inits2 <- list("alpha0" = c(log(sum(y==1)/length(y)/(sum(y!=1)/length(y))), log(sum(y<=2)/length(y)/(sum(y>2)/length(y)))), "beta" = rep(0,P))
  inits3 <- list("alpha0" = sort(c(rnorm(1,log(1/2),.5), rnorm(1,log(2/1),.5))), "beta" = rep(0,P))
  
  start.time.1 <- Sys.time()
  parJagsModel(cl, name = "test", file = model1, 
  ...
  BFbeta.1 = (1:P)[BetgmaFactor>10] # let threshold = 10 for bayes factor
  ...
  e.ci.1<-ifelse( (ci[1,]>0 & ci[2,]>0) | (ci[1,]<0 & ci[2,]<0) ,TRUE,FALSE)
   if (sum(e.ci.1)>0) {
    e.ci.id.1<-(1:P)[e.ci.1]
   }
     ### select variable using 95% HPD credible interval of gammaXbeta
   hd<-apply(betamatrix,2,hdi,0.95)
   e.hd.1<-ifelse( (hd[1,]>0 & hd[2,]>0) | (hd[1,]<0 & hd[2,]<0) ,TRUE,FALSE)
   if (sum(e.hd.1)>0) {
    e.hd.id<-(1:P)[e.hd.1]
   }
 save.image("Model1BMC.RData")
  stopCluster(cl)
```
