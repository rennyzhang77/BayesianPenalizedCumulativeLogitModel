---
title: "Bayesian Variable Selection: Model 3 Fixed Prior 0.50"
...
N <- dim(expression)[2]
P <- dim(expression)[1]
p = 0.50 # calculate prior probability for gamma
lambda0 = 20 # using spike-and-slab LASSO prior for beta, lambda0 is the hyperparameter for the laplace distribution when gamma = 0 
...
### Proposed Bayesian Model III with Fixed prior for pi
model3<- function(){
  for(i in 1:N){
  ...
  for (j in 1:P){
    ...
    gamma[j] ~ dbern(0.50) 
    DE1[j] ~ ddexp(0, 20) # variance = 0.005
...
nIter <- ceiling((numSavedSteps*thinSteps )/nChains) 	#steps per chain
  ...
  params <- c("beta", "gamma")
    ...
    inits1 <- list("alpha0" = sort(c(rnorm(1,log(1/2),.5), rnorm(1,log(2/1),.5))), "DE1" = rep(0,P), "DE2" = rep(0,P), "pi" = rep(0,P))
  inits2 <- list("alpha0" = c(log(sum(y==1)/length(y)/(sum(y!=1)/length(y))), log(sum(y<=2)/length(y)/(sum(y>2)/length(y)))), "DE1" = rep(0,P), "DE2" = rep(0,P), "pi" = rep(0,P))
  inits3 <- list("alpha0" = sort(c(rnorm(1,log(1/2),.5), rnorm(1,log(2/1),.5))), "DE1" = rep(0,P), "DE2" = rep(0,P), "pi" = rep(0,P))
  
    start.time.3.pi.50 <- Sys.time()
    parJagsModel(cl, name = "test", file = model3, 
...
### select variable using top 10 gamma mean's
  ...
  gamma10.index.3 = sort(gammamean, decreasing = T, index.return =T)$ix[1:10]
...
### select variable using bayes factor based on gamma
  ...
  gamma.index.3 = (1:P)[PostOdds/PriorOdds > 10] # using threshold = 10 for bayes factor
  ...
  PostOdds2 = BetgmaPost1/BetgmaPost2
   PriorOdds2 = (p*b^a*gamma(a+1)+(1-p)*a*gamma(a)*(b+epsilon)^a*exp(-lambda0*epsilon))/(p*gamma(a+1)*((b+epsilon)^a-b^a)+(1-p)*a*gamma(a)*(1-exp(-lambda0*epsilon))*(b+epsilon)^a)
  BetgmaFactor = PostOdds2/PriorOdds2 # calculate bayes factor for Model III
  BFbeta.3 = (1:P)[BetgmaFactor>10] # let threshold = 10 for bayes factor
  ...
  e.ci.3<-ifelse( (ci[1,]>0 & ci[2,]>0) | (ci[1,]<0 & ci[2,]<0) ,TRUE,FALSE)
   if (sum(e.ci.3)>0) {
    e.ci.id.3<-(1:P)[e.ci.3]
   }
     ### select variable using 95% HPD credible interval of gammaXbeta
   hd<-apply(betamatrix,2,hdi,0.95)
   e.hd.3<-ifelse( (hd[1,]>0 & hd[2,]>0) | (hd[1,]<0 & hd[2,]<0) ,TRUE,FALSE)
   if (sum(e.hd.3)>0) {
    e.hd.id.3<-(1:P)[e.hd.3]
   }
save.image("Model3FixedPi50BMC.RData")
stopCluster(cl)
```
