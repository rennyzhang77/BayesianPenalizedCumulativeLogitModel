...
### Simulation Study Example: using Beta(1, 19) prior for pi, number of feature p = 1500, sample size n = 99, 
...
p = beta(2,19)/beta(1,19) # calculate prior probability for gamma
...
### Proposed Bayesian Penalized Cumulative Logit Model with Beta(1, 19) prior for pi
model<- function(){
  for(i in 1:N){
...
for (j in 1:Nsim){
  ...
  PriorOdds = p/(1-p)
  gamma.index = (1:P)[PostOdds/PriorOdds > 5] # using threshold = 5 for bayes factor
...
save.image("SimulationExample2.RData")
