rm(list=ls())
#setwd("/Users/YiranZhang/Documents/STAT_server/NewSim")
#require("rjags")
require("HDInterval")
require("MASS")
require("Matrix")
...
b = 0.1
N=99 # define sample size
P=1500 # define number of covariates
p = 0.05 # calculate prior probability for gamma
Nsim = 20 # define number of replications
...
sigma = bdiag(CovBlock,CovBlock,CovBlock,CovBlock,CovBlock,CovBlock,CovBlock,CovBlock,CovBlock,CovBlock,
              CovBlock,CovBlock,CovBlock,CovBlock,CovBlock,CovBlock,CovBlock,CovBlock,CovBlock,CovBlock,
              CovBlock,CovBlock,CovBlock,CovBlock,CovBlock,CovBlock,CovBlock,CovBlock,CovBlock,CovBlock) # this is for P=500, add two extra lines for P=1500

### Define value of coefficients
# true.beta = matrix(0, P, 2)
true.beta = c(rep(0,P))
...
# for(i in true.index) {
#   true.beta[i,]<-sample(c(log(3), log(4)), 2, replace=FALSE)*rep(sample(c(+1,-1),1),2)
# }
true.beta[true.index] = c(rep(-log(3), ceiling(Nzero/2)), rep(log(3), Nzero - ceiling(Nzero/2)))
alpha = c(-1.8, 2)

### Function calculate cumulative probability
cum_prob <- function(alpha, true.beta, X, k){
  exp(alpha[k] - X%*%true.beta)/(1+exp(alpha[k] - X%*%true.beta)) 
} 

### Proposed Bayesian Model II Fixing pi = 0.05
model <- function(){
  for(i in 1:N){
...
for (j in 1:Nsim){
  ...
  prob_1 = cum_prob(alpha, true.beta, X, 1)
  prob_2 = cum_prob(alpha, true.beta, X, 2) - cum_prob(alpha, true.beta, X, 1)
  prob_3 = 1 - cum_prob(alpha, true.beta, X, 2)
  y = matrix(0,N,1)
  for (i in 1:N) {
    y[i,] = sample(
      x = c(1:3), 
      size = 1, 
      prob = c(prob_1[i], prob_2[i], prob_3[i])
    )
  }
  k<-length(unique(y))
  Y[,j] = y
  ...
  PriorOdds = p/(1-p)
  gamma.index = (1:P)[PostOdds/PriorOdds > 1.5] # let threshold = 1.5 for bayes factor
  ...
  BetgmaFactor = PostOdds/PriorOdds # calculate bayes factor for Model II
  BFbeta = (1:P)[BetgmaFactor>1.5] # let threshold = 1.5 for bayes factor
  if(length(BFbeta)>0){
...
save.image("UnequalLevels_99_1.RData")
