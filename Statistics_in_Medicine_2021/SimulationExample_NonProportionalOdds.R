...
### This is a simulation study example when the proportional odds assumption is violated.
### Simulation Example: letting prior pi = 0.05, number of feature p = 1500, sample size n = 99, 
...
cum_prob <- function(alpha, true.beta, X, k){
  exp(alpha[k] - X%*%true.beta[,k])/(1+exp(alpha[k] - X%*%true.beta[,k])) 
} 
...
save.image("SimulationNonProportionalOdds.RData")
